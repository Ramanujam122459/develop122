...
def save_test_plan(test_plan, file_path):
    ...
    with open(file_path, "w", encoding="utf-8") as file:
        ...
        file.write(f"- **Test Name:** {test_plan.get('test_name', '')}\n")
        file.write(f"- **Test Type:** {test_plan.get('test_type', '')}\n\n")

        # Target
        file.write("## Target\n\n")
        file.write(f"- **URL:** {test_plan.get('target_url', '')}\n")
        file.write(f"- **HTTP Method:** {test_plan.get('http_method', '')}\n\n")
        ...
        if "vus" in load_profile:
            file.write(f"- **VUs:** {load_profile.get('vus', '')}\n")

        if "duration" in load_profile:
            file.write(f"- **Duration:** {load_profile.get('duration', '')}\n")
        ...
        if stages:
            ...
            for stage in stages:
                if isinstance(stage, dict):
                    ...
                    file.write(f"  - {duration} → {target} VUs\n")
        file.write("\n")
        ...
        file.write(
            f"- **P95 Response Time:** {thresholds.get('p95_response_time_ms', '')} ms\n"
        )
        file.write(
            f"- **Error Rate:** {thresholds.get('error_rate_percent', '')}%\n\n"
        )
...
def main():
    ...
    # ==========================================
    # Set Performance Test Scenario (Default: Load)
    # ==========================================
    test_config["scenario"] = "load"
    print(f"\n✅ Selected Performance Test: {test_config['scenario'].upper()}")
...
"""
    ...
    save_test_plan(test_plan, test_plan_file)
    ...
    GENERATED_SCRIPT.write_text(reviewed_script, encoding="utf-8")
    ...
    analysis = analyzer.run(execution_result, test_plan)
    ...
    report = reporter.run(test_plan, analysis)
